...
func newHtlcResolution(signer Signer, localChanCfg *channeldb.ChannelConfig,
	...
	delayTweak := SingleTweakBytes(keyRing.commitPoint,
...
func extractHtlcResolutions(feePerKw btcutil.Amount, ourCommit bool,
	...
	for _, htlc := range htlcs {
		...
		ohr, err := newHtlcResolution(signer, localChanCfg, commitHash,
			htlc, keyRing, feePerKw, dustLimit, uint32(csvDelay))
		if err != nil {
			return nil, err
		}
...
func deriveCommitmentKeys(commitPoint *btcec.PublicKey, isOurCommit bool,
	...
	keyRing.revocationKey = DeriveRevocationPubkey(revocationBasePoint,
		commitPoint)
